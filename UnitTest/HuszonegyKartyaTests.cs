...
namespace UnitTest
{
    ...
    public class HuszonegyKartyaTests
    {
        ...
        public void HuszonegyKartyaStartMFranciaTest()
        {
            ...
            Assert.IsTrue(ui.TestSteps.Count == 21, "Túl sok vagy kevés az output");
            Assert.IsTrue(ui.TestSteps[0].StartsWith("Sound"), "Nem a zene lejátszásával kezdődik");
            ...
            Assert.IsTrue(ui.TestSteps[5].StartsWith("Sound"), "Nem a zene lejátszásával végződik");
            Assert.IsTrue(ui.TestSteps[5].Contains("Step"), "Nem a Step zene kerül lejátszásra");

            Assert.IsTrue(ui.TestSteps[7].StartsWith("Clear"), "Nem történik képernyő törlése");
            Assert.IsTrue(ui.TestSteps[8].Contains("Válassz kártya paklit."), "Nem lett kiírva a választási lehetőség");

            Assert.IsTrue(ui.TestSteps[10].StartsWith("Sound"), "Nem a zene lejátszásával végződik");
            Assert.IsTrue(ui.TestSteps[10].Contains("Step"), "Nem a Step zene kerül lejátszásra");

            Assert.IsTrue(ui.TestSteps[11].StartsWith("Clear"), "Nem történik képernyő törlése");
            Assert.IsTrue(ui.TestSteps[12].Contains("Választott pakli:"), "Nem lett kiírva a választott pakli");
            Assert.IsTrue(ui.TestSteps[13].Contains("Dáma"), "Nem lett kiírva a lapok értéke");
            Assert.IsTrue(ui.TestSteps[14].Contains("A játékszabályok:"), "Nem lettek kiírva a játékszabályok");
            Assert.IsTrue(ui.TestSteps[15].Contains("Ha az osztó és játékos lapjainak értéke egyenlő a játékos visszakapja a tétet!"), "Nem lettek kiírva a játékszabályok");

            Assert.IsTrue(ui.TestSteps[16].Contains("darab játékos játsza"), "Nem lett kiírva a játékosok száma és a paki tipusa");
            Assert.IsTrue(ui.TestSteps[17].Contains("Kezdődjön a játék!"), "Nem lett kiírva a kezdés jelzése");
            Assert.IsTrue(ui.TestSteps[20].StartsWith("Sound"), "Nem a zene lejátszásával végződik");
            Assert.IsTrue(ui.TestSteps[20].Contains("Good"), "Nem a Good zene kerül lejátszásra");
        }
        [TestMethod]
        public void HuszonegyKartyaStartMagyarTest()
        {
            ...
            Assert.IsTrue(ui.TestSteps[11].StartsWith("Clear"), "Nem történik képernyő törlése");
            Assert.IsTrue(ui.TestSteps[12].Contains("Választott pakli:"), "Nem lett kiírva a választott pakli");
            Assert.IsTrue(ui.TestSteps[13].Contains("Felső"), "Nem lett kiírva a lapok értéke");
            Assert.IsTrue(ui.TestSteps[14].Contains("A játékszabályok:"), "Nem lettek kiírva a játékszabályok");
            Assert.IsTrue(ui.TestSteps[15].Contains("Ha az osztó és játékos lapjainak értéke egyenlő a játékos vesztett!"), "Nem lettek kiírva a játékszabályok");
        ...
        public void HuszonegyKartyaResultBustTest()
        {
            ...
            Assert.IsTrue(ui.TestSteps[2].StartsWith("Sound"), "Nem a zene lejátszásával indul");
            Assert.IsTrue(ui.TestSteps[2].Contains("Bad"), "Nem a Bad zene kerül lejátszásra induláskor");
        }
        [TestMethod]
        public void HuszonegyKartyaResultSurrenderTest()
        {
            ...
            Assert.IsTrue(ui.TestSteps[3].Contains("Bad"), "Nem a Bad zene kerül lejátszásra induláskor");
        }
        [TestMethod]
        public void HuszonegyKartyaResultWinTest()
        {
            ...
            Assert.IsTrue(ui.TestSteps[1].Contains("fg:DarkBlue|bg:Cyan"), "Nem megfelelő a szinek beállítása");
            Assert.IsTrue(ui.TestSteps[2].StartsWith("Sound"), "Nem a zene lejátszásával indul");
            Assert.IsTrue(ui.TestSteps[2].Contains("Good"), "Nem a Good zene kerül lejátszásra induláskor");
        }
        [TestMethod]
        public void HuszonegyKartyaResultBlackjackTest()
        {
            ...
            Assert.IsTrue(ui.TestSteps[1].Contains("fg:DarkBlue|bg:Cyan"), "Nem megfelelő a szinek beállítása");
            Assert.IsTrue(ui.TestSteps[2].StartsWith("Sound"), "Nem a zene lejátszásával indul");
            Assert.IsTrue(ui.TestSteps[2].Contains("Win"), "Nem a Win zene kerül lejátszásra induláskor");
        }
        [TestMethod]
        ...
        public void HuszonegyKartyaResultDontetlenTest()
        {
            ...
            Assert.IsTrue(ui.TestSteps[2].Contains("fg:DarkBlue|bg:Cyan"), "Nem megfelelő a szinek beállítása");
            Assert.IsTrue(ui.TestSteps[3].StartsWith("Sound"), "Nem a zene lejátszásával indul");
            Assert.IsTrue(ui.TestSteps[3].Contains("Tie"), "Nem a Tie zene kerül lejátszásra induláskor");
        }
        [TestMethod]
        public void HuszonegyKartyaEndTest()
        {
            ...
            Assert.IsTrue(ui.TestSteps[1].Contains("fg:Black|bg:White"), "Nem megfelelő a szinek beállítása");
            Assert.IsTrue(ui.TestSteps[2].Contains("Nincs elegendő Coin."), "Rossz eredmény lett kiírva");
            Assert.IsTrue(ui.TestSteps[3].Contains("A játéknak vége!"), "Rossz eredmény lett kiírva");
        }
    }
}
